...
function onGetDeviceDescription(callbackObject, applicationUrl, didRedirect,
    ...
    if (didRedirect === false) {
        testAPI.reportStepResult(5, true, "didRedirect was equal to " +
            didRedirect);
    }
    else {
        testAPI.reportStepResult(5, false, "didRedirect was equal to " +
            didRedirect);
        return;
    }
    ...
    testAPI.dial.getHbbtvAppDescription(applicationUrl, true, null,
        getHbbtvAppDescriptionCallback);
}

function getHbbtvAppDescriptionCallback() {
    testAPI.reportStepResult(6, true, "getHbbtvAppDescriptionCallback was " +
        "called");
