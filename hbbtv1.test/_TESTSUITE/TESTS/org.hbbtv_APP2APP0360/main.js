...
function openLocalWebSocket(url) {
    ...
    onmessage = function (event) {
        ...
        if (event.data === 'pairingcompleted') {
            ...
            clearTimeout(sendMessageTimeout);
            if (checkPayload(event.data)) {
                testAPI.reportStepResult(4, true, "Local websocket received " +
                    "binary message of length " + event.data.length +
...
function onLocalAndRemoteWebSocketsPaired() {
    ...
    remoteWebSocket.sendMessage(messagePayload, true);
}
...
function onSendMessageTimeout() {
    testAPI.reportStepResult(4, false, "Timeout sending message payload");
    tearDown();
}

function createPayload(count) {
    var result = String.fromCharCode(65, 66, 67, 68, 69, 70); // "ABCDEF"
...
function checkPayload(string) {
    var charCodes = [65, 66, 67, 68, 69, 70]; // "ABCDEF"
    for (var i = 0; i < string.length; i++) {
        if (charCodes[i % charCodes.length] !== string.charCodeAt(i)) {
            return false;
        }
    }
    return true;
