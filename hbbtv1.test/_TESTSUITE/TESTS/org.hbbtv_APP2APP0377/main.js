...
var appEndPoint = "myapp.mychannel.org/org.hbbtv_APP2APP0377";
var numberOfSocketsToOpen = 10;
var messageSendDuration = 60000;
var messageLength = 512;
var messageRepeatFrequency = 400;
...
function initApp2AppComms(localURL, remoteURL) {
    ...
    try {
        for (i = 0; i < numberOfSocketsToOpen; i++) {
            ...
            // Save the local sockets and ids as we'll them for message sending
...
function localWebSocketOnMessage(event, id, messageCount) {
    ...
    testAPI.reportStepResult(3, false, "Local Websocket received " +
        ...
        " received on any local Websockets");
    tearDown();
}
...
function sendMessage() {
    ...
    for(var i = 0; i < webSocketClients.length; i++) {
        ...
        // Send the message through the local web socket
        websocket.client.send(messagePayload, false);
